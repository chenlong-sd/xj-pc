...
;(function ($, window, document, undefined) {
  ...
  Calendar.prototype = {
    ...
    showCalendar: function () { // 输入数据并显示
      vm.go(returnDateStr(new Date()).replace(/^(\d{4})(\d{2})(\d{2})$/, "$1-$2-$3"));
      var self = this;
      ...
      this.$calendarDate_item.each(function (i) {
        ...
        } else if (returnDateStr(firstDay).substr(0, 6) === allDay_str.substr(0, 6) && allDay_str > returnDateStr(new Date())) {
    ...
    renderDOM: function () { // 渲染DOM
      ...
      this.$calendar_week = $('<div class="calendar-week"></div>');
      this.$calendar_date = $('<div class="calendar-date"></div>');
      this.$calendar_today = $('<div class="calendar-today"></div>');


      var _titleStr = '<span class="arrow-prev"><</span>'+'<a href="#" class="title"></a>'+
          '<span class="arrow-next">></span>'
       ;
      var _weekStr = '<div class="item">日</div>'+
      ...
      for (var i = 0; i < 5; i++) {
        _dateStr += '<div class="item">26</div>'+
    ...
    inital: function () { // 初始化
      ...
      this.$calendarDate_item.hover(function () {
        let clickTime = $(this).attr('data');
        if (clickTime >= returnDateStr(new Date())) {
          vm.go($(this).attr('data').replace(/(\d{4})(\d{2})(\d{2})/,"$1-$2-$3"))
        }
