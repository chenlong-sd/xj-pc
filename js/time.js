...
$.extend({
    selectYY_MM_DD: function (g, a, k) {
        ...
        new MobileSelect({
            trigger: g, title: '閫夋嫨鏃堕棿', wheels: function (a, b) {
                ...
                return d
            }(d, e), position: b, callback: function (a, b) {
                if (k != undefined) {
                    k({year: b[0].id, month: b[1].id, day: b[2].id})
                }
            }
        })
        ...
        var createDateData = function (info) {
            ...
            for (var j = 1; j <= 12; j++) {
                if (j < 10) {
                    childData.push({id: '0' + j, value: '0' + j});
                } else {
                    childData.push({id: j, value: j});
                }
            }
            for (var i = info.startYear; i <= info.maxYear; i++) {
                dateData[0].data.push({id: i, value: i, childs: childData});
            }
            return dateData;
        };
        var info = {}, now = new Date();
        ...
        var createDateData = function (info) {
            ...
            if ($(el).data("type") == "end") dateData[0].data.push({id: info.maxYear + 1, value: "鑷充粖"});
            return dateData;
        };
        var info = {}, now = new Date();
        info.nowYear = now.getFullYear();
        ...
        var createDateData = function (info) {
            var dateData = [{data: []}];
            for (var i = info.startYear; i <= info.maxYear; i++) {
                dateData[0].data.push({id: i, value: i});
            }
            return dateData;
        };
        var info = {}, now = new Date();
        ...
        var createDateData = function (info) {
            ...
            for (var j = 0; j <= 59; j++) {
                if (j < 10) {
                    childDatass.push({id: '0' + j, value: '0' + j});
                } else {
                    childDatass.push({id: j, value: j});
                }
            }
            for (var j = 0; j <= 59; j++) {
                if (j < 10) {
                    childData.push({id: '0' + j, value: '0' + j, childs: childDatass});
                } else {
                    childData.push({id: j, value: j, childs: childDatass});
                }
            }
            for (var i = info.startHour; i <= info.maxHour; i++) {
                var pra = i < 10 ? '0' + i : i
